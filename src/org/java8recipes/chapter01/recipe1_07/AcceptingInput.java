 ...
 * Solution:
 *
 * Make use of the java.io.BufferedReader and java.io.InputStramReader classes to read keyboard entry and store
 * it into local variables. Listing 1-7 shows a program that will keep propting for input until you enter some characters
 ...
 * Syste.in provides access to raw bytes of data from the input device, which is the keyboard in our example. It is
 * the job of the InputStreamReader class to take tose bytes and convert them into characters in your current character
 * set. System.in is passed to the InputStreamReader() constructor to create an InputStreamReader object.
 * InputStramReader knows about characters, but not about lines. It is the BufferedReader class's job to detect
 * line break's in the input stream, and to enable you to conveniently red a line at a time. BufferedReader also aids
 * efficiency by allowing physical reads from the input device to be done in different-size chunks than by which your
 * application consumes the data. This aspect can make a difference when the input stram is a large file rather than the
 * keyboard.
